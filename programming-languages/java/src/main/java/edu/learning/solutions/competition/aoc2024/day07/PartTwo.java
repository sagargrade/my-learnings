...
class PartTwo {
    ...
    private boolean isEquationValid(List<Long> values, Long result) {
        ...
        for (char[] combination : operatorCombination) {
            ...
            for (int i = 1; i < values.size(); i++) {
                ...
                } else if (combination[i - 1] == Operator.MULTIPLY.getSign()) {
                    calculatedValue *= values.get(i);
                } else {
                    calculatedValue = Long.parseLong(calculatedValue + "" + values.get(i));
                }
            }
    ...
    private List<char[]> determineOperatorCombination(int numberOfOperators) {
        ...
        char[] operators = new char[]{Operator.PLUS.getSign(), Operator.MULTIPLY.getSign(), Operator.CONCATENATE.getSign()};
